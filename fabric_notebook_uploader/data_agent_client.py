...
A client for calling Microsoft Fabric Data Agents from outside of the Fabric
environment using interactive browser authentication. This is integrated into
the fabric_notebook_uploader project for testing data agents locally.

Based on the Microsoft fabric_data_agent_client:
...
def _load_env_file():
    ...
    try:
        ...
        project_root = os.path.dirname(script_dir)  # Parent of fabric_notebook_uploader module
...
class FabricDataAgentClient:
    ...
    - SQL query extraction and data preview for lakehouse sources
    """
    ...
    def get_run_details(self, question: str) -> Dict:
        """
        Ask a question and return detailed run information including steps and SQL queries.
    ...
    def _extract_data_from_text_response(self, text_content: str) -> List[str]:
        """
        Extract structured data from the assistant's text response.
    ...
    def _format_list_data(self, data_list) -> List[str]:
        """
    ...
    def _extract_data_preview(self, text: str) -> List[str]:
        """
    ...
    def _extract_sql_queries(self, steps) -> List[str]:
        """
    ...
    def _find_sql_in_text(self, text: str) -> List[str]:
        """
...
def create_data_agent_client_from_env() -> Optional[FabricDataAgentClient]:
    """
    Create a FabricDataAgentClient using environment variables or .env file.
    
    Returns:
        FabricDataAgentClient instance or None if configuration is missing
    """
    tenant_id = os.getenv("TENANT_ID")
    data_agent_url = os.getenv("DATA_AGENT_URL")
    
    if not tenant_id or not data_agent_url:
        print("❌ Missing configuration. Please set TENANT_ID and DATA_AGENT_URL environment variables")
        print("\nOptions:")
